...
async function api(path,body){return window.guiltypleasureDemo.request(path,body)}
...
function navigation(){splitCleanup();document.body.classList.remove('home-view');stopBirthdayRotation(); $('#nav').innerHTML=button('Close People','groups',page==='groups'?'aria-current="page"':'','')+button('Gift List','giftlist',page==='list'?'aria-current="page"':'','')+button('Demo account','account','','');$('#account').textContent=''; }
...
account:()=>dialog(`<h2>Your little corner.</h2><p>${previewMode?'Interactive demo. Your changes stay in this browser only. People, gifts and dates are fictional; no emails or real invitations are sent.':esc(user)}</p><div class="actions">${button('Close','close')}${button('Sign out','logout','','primary')}</div>`),
...
copy:async()=>{await navigator.clipboard.writeText(location.origin+location.pathname+'?list='+current.id);toast('Demo link copied. Changes in this browser are not shared.')},
